...
DEVICE = "cuda"
MY_CONFIG_PATH="/data1/jungmyungwi/ICML2026/reward-server/mmdetection/configs/mask2former/mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco.py" #"your_mmdetection_path/mmdetection/configs/mask2former"
MY_CKPT_PATH="/data1/jungmyungwi/ICML2026/reward-server/reward_server/model/mask2former2" #"your_reward-server_path/reward-server/model/mask2former2"
...
def load_geneval():
    ...
    def load_models():
        ...
        OBJECT_DETECTOR = "mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco"
    ...
    def evaluate_image(image_pils, metadatas, only_strict):
        ...
        for result, image_pil, metadata in zip(results, image_pils, metadatas):
            ...
            if hasattr(result, 'pred_instances'):
                # v3 버전인 경우: 객체에서 데이터를 꺼내 v2 형식(List of Arrays)으로 재조립
                pred_instances = result.pred_instances.cpu().numpy()
                all_bboxes = pred_instances.bboxes
                all_scores = pred_instances.scores
                all_labels = pred_instances.labels
                
                # 마스크가 있는지 확인
                has_masks = 'masks' in pred_instances
                
                bbox = []
                segm = [] if has_masks else None
                
                # classnames 개수만큼 리스트를 미리 만들어야 함 (인덱스로 접근하므로)
                for i in range(len(classnames)):
                    # 해당 클래스(i)에 맞는 인덱스만 골라내기
                    inds = (all_labels == i)
                    
                    if np.any(inds):
                        # [x1, y1, x2, y2] + [score] 합쳐서 (N, 5) 배열 만들기
                        cls_bbox = np.concatenate([all_bboxes[inds], all_scores[inds][:, None]], axis=1)
                        bbox.append(cls_bbox)
                        if has_masks:
                            segm.append(pred_instances.masks[inds])
                    else:
                        # 해당 클래스 물체가 없으면 빈 배열 추가
                        bbox.append(np.zeros((0, 5), dtype=np.float32))
                        if has_masks:
                            segm.append(np.zeros((0, *image_pil.size[::-1]), dtype='uint8')) # dummy mask
            else:
                # v2 버전인 경우 (혹시 모를 하위 호환성)
                bbox = result[0] if isinstance(result, tuple) else result
                segm = result[1] if isinstance(result, tuple) and len(result) > 1 else None
