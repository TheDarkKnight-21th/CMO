...
def geneval_sd3_fast_nocfg_4gpu():
    gpu_number = 4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
...
def geneval_sd3_fast_nocfg_1gpu():
    gpu_number = 1
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
    ...
    config.save_freq = 60 # epoch
    config.eval_freq = 60
    config.save_dir = 'logs/ours/sd3.5-M-gdpo-fast-1gpu'
    config.reward_fn = {
        "geneval": 1.0,
    }
    ...
    config.per_prompt_stat_tracking = True
    return config
def ours_sd3_fast_4gpu_geneval():
    gpu_number = 4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
    ...
    config.train.num_inner_epochs = 1
    config.train.clip_range = 1e-5
    config.train.beta = 0.015
    config.sample.global_std = True
    config.sample.same_latent = False
    config.sample.noise_level = 0.8
    config.sample.sde_window_size = 3
    config.sample.sde_window_range = (0, config.sample.num_steps//2)
    config.sample.fix_window = (0, 3)
    config.sample.sde_type = "cps"
    config.train.ema = True
    config.save_freq = 60 # epoch
    config.eval_freq = 60
    config.save_dir = 'logs/ours/sd3.5-M-gdpo-fast-4gpu'
    config.reward_fn = {
        "ours": 1.0,
    }

    config.train.temperature = 0.5
    
    config.prompt_fn = "ours_geneval"

    config.train.attr_binding = False
    
    config.per_prompt_stat_tracking = True

    return config

def ours_sd3_fast_4gpu():
    gpu_number = 4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
    ...
    config.prompt_fn = "ours"

    config.train.attr_binding = True
    
    config.per_prompt_stat_tracking = True
    return config
def conceptmix_flux_fast_4gpu():
    gpu_number = 4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
    ...
    config.eval_freq = 30
    config.save_dir = 'logs/conceptmix/flux_fast'
    config.reward_fn = {
        "conceptmix": 1.0,
    }
    
    config.prompt_fn = "conceptmix"
    config.train.attr_binding = True

    config.per_prompt_stat_tracking = True
    return config

def conceptmix_sd3_fast_4gpu():
    gpu_number = 4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/geneval")
    ...
    config.save_freq = 60 # epoch
    config.eval_freq = 60
    config.save_dir = 'logs/conceptmix/sd3.5-M-gdpo-fast-4gpu'
    config.reward_fn = {
        "conceptmix": 1.0,
    }
...
def geneval_flux2_fast_4gpu():
    ...
    config.sample.sde_window_range = (0, config.sample.num_steps//2)
    config.sample.sde_type = "cps"
    config.sample.fix_window = None #(0, 2)
    config.train.ema = True
    config.mixed_precision = "bf16"
    config.save_freq = 60 # epoch
    config.eval_freq = 60
    config.save_dir = 'logs/geneval/flux2klein_fast'
    config.reward_fn = {
        "geneval": 1.0,
    }
...
def ours_flux_fast_4gpu():
    ...
    config.save_freq = 30 # epoch
    config.eval_freq = 30
    config.save_dir = 'logs/ours/flux_fast'
    config.reward_fn = {
        "ours": 1.0,
    }
