...
def ours_score(device):
    ...
    def _fn(images, prompts, metadatas):
        # del prompts # 프롬프트는 메타데이터에 포함되어 있거나, 리워드 계산 시 메타데이터 위주로 사용됨

        # 1. Image Preprocessing (Tensor -> Numpy (NHWC) -> PIL -> JPEG Bytes)
        if isinstance(images, torch.Tensor):
            # (N, C, H, W) -> (N, H, W, C)
            images = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
        ...
        # 3. Result Containers (서버에서 반환하는 3가지 데이터에 맞춰 변경)
        all_reward_tensors = []          # List of Lists (Flat Tensors)
        all_object_details = []          # List of Dicts
        all_spatial_details = []         # List of Lists

        # breakpoint() # 디버깅 필요 시 주석 해제

        for image_batch, metadata_batched, prompt_batched in zip(images_batched, metadatas_batched, prompts_batched):
            jpeg_images = []

            # Compress images to JPEG for transmission
            for image in image_batch:
                img = Image.fromarray(image)
                buffer = BytesIO()
                img.save(buffer, format="JPEG")
            ...
            # 4. Prepare Payload
            # 서버의 inference() 함수가 pickle.loads(data)를 수행하고, 
            # 내부 키로 'images', 'meta_datas'를 참조함
            data = {
                "images": jpeg_images,
                "meta_datas": list(metadata_batched),
                "prompts": list(prompt_batched),
                # "only_strict": False # 우리 로직에는 strict 모드가 없으므로 생략 가능
            }
            data_bytes = pickle.dumps(data)
            ...
            try:
                ...
                # 6. Unpack Response
                # 서버가 반환한 키: 'reward_tensors', 'detailed_object_rewards', 'detailed_spatial_scores'
                response_data = pickle.loads(response.content)

                all_reward_tensors.extend(response_data["reward_tensors"])
                # all_object_details.extend(response_data["detailed_object_rewards"])
                # all_spatial_details.extend(response_data["detailed_spatial_scores"])
            
            except Exception as e:
                print(f"[Error] Request failed: {e}")
                # 에러 발생 시 빈 리스트나 0.0을 채워 넣는 예외 처리가 필요할 수 있습니다.
                # 여기서는 간단히 pass 처리합니다.
                pass


        # 7. Final Return
        # 학습에 필요한 텐서 리스트와 로그용 디테일 정보를 반환합니다.
        return all_reward_tensors, all_object_details, all_spatial_details

    return _fn

def conceptmix_score(device):
    """Submits images to GenEval and computes a reward.
    """
    import requests
    from requests.adapters import HTTPAdapter, Retry
...
def multi_score(device, score_dict):
    ...
    def _fn(images, prompts, metadata, ref_images=None, only_strict=True):
        ...
        for score_name, weight in score_dict.items():
            ...
            elif score_name == "ours" or score_name == "conceptmix":
                # ours_score는 3개의 값을 반환함 (Tensors, Object Details, Spatial Details)
                # reward_tensors: List of Lists (Batch x N attributes)
                reward_tensors, obj_details, spat_details = score_fns[score_name](images, prompts, metadata)
                
                # [중요] total_scores 계산을 위해 텐서의 '평균값'을 대표 스코어로 사용
                # (GRPO Trainer가 스칼라 합산을 원할 경우를 대비)
                # 상세 정보는 score_details에 따로 저장 (로그용)
                score_details['scores'] = reward_tensors
                # score_details['ours_object_details'] = obj_details
                # score_details['ours_spatial_details'] = spat_details
