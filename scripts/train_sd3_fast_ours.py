...
from flow_grpo.diffusers_patch.sd3_pipeline_with_logprob_fast_ours import pipeline_with_logprob
    ...
    def __init__(self, 
        ...
        def load_data(c_path, s_path):
            ...
            for idx in common:
                ...
                if 'questions' in s_map[idx].keys():
                    meta['questions'] = s_map[idx]['questions']
...
def main(_):
    ...
    config.save_dir += "_" + config.run_name 


    if config.sample.fix_window is not None:
        num_train_timesteps = config.sample.fix_window[-1]
    elif config.sample.sde_window_size > 0:
        num_train_timesteps = config.sample.sde_window_size
    else:
        num_train_timesteps = config.sample.num_steps - 1
    config.run_name += "_gdpo_fast_hyperconceptmix"
    if config.prompt_fn == 'conceptmix':
        config.run_name += "_conceptmix_reward"
        config.save_dir += "_conceptmix_reward"
    ...
    elif config.prompt_fn == "conceptmix":
        train_dataset = BalancedGRPODataset(
            config_path_a="./dataset/hyperconcepmix_train/3000_refined_config_k=7.json", 
            sentence_path_a="./dataset/hyperconcepmix_train/3000_sentences_v2_k=7.json",
            config_path_b="./dataset/hyperconcepmix_train/final_config_all.json", 
            sentence_path_b="./dataset/hyperconcepmix_train/final_sentences_v2_all.json"
        )
    ...
    def print_trainable_parameters(models: dict):
        """
        Print trainable parameter stats for multiple models.
        Args:
            models (dict): {name: model} 형태의 딕셔너리
        """
        for name, model in models.items():
            total_params = sum(p.numel() for p in model.parameters())
            trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            print(
                f"{name}: 학습 가능 params = {trainable_params:,} / {total_params:,} "
    ...
    if accelerator.is_main_process:
        models_to_check = {
            ...
            "Text Encoder2": pipeline.text_encoder_2,
            "Text Encoder3": pipeline.text_encoder_3,
            "VAE": pipeline.vae
        }

        print_trainable_parameters(models_to_check)
    ...
    if config.sample.same_latent:
        print("Using same latents for each prompt during sampling")
        global_input_latents = torch.randn(
            (1, pipeline.transformer.config.in_channels, config.resolution // pipeline.vae_scale_factor, config.resolution // pipeline.vae_scale_factor),
            device="cuda",
            generator = torch.Generator("cuda").manual_seed(2026)
        ).repeat(config.train.batch_size, 1, 1, 1)
        generator= None
    else:
    ...
    while True:
        ...
        for i in tqdm(
            ...
            if config.prompt_fn == "ours" or config.prompt_fn == "conceptmix":
            ...
            with autocast():
                ...
                with torch.no_grad():
                    images, latents, log_probs, timesteps = pipeline_with_logprob(
                        ...
                        sde_type=config.sample.sde_type,
                        fix_window=config.sample.fix_window,
                        latents=global_input_latents
                )
            ...
            timesteps = torch.stack(timesteps).unsqueeze(0).repeat(config.sample.train_batch_size, 1)
        ...
        for sample in tqdm(
            ...
            for key, value in rewards.items():
                ...
                for v in value:
                    ...
                    t = torch.as_tensor(v, device=accelerator.device).float()
                    ...
                    if current_len < max_reward_len:
                        ...
                        t = F.pad(t, (0, diff), value=-1.0)
                        
                    padded_tensors.append(t)
                    

                sample["rewards"][key] = torch.stack(padded_tensors)
        ...
        if config.per_prompt_stat_tracking:
            ...
            advantages = stat_tracker.update(prompts, dummy_rewards, 
                                            reward_tensors=gathered_rewards['scores'],
                                            attr_binding=config.train.attr_binding,
