...
def main(_):
    ...
    config.run_name += "_flux_fast_hyperconceptmix_hps" 
    ...
    while True:
        ...
        max_reward_len = train_dataset.global_max_reward_count + 1
        for s in samples:
        ...
        samples = {
            ...
            if not isinstance(samples[0][k], dict)
                ...
                sub_key: torch.cat([s[k][sub_key] for s in samples], dim=0).float().squeeze(1)
        ...
        ori_scores = gathered_rewards['ori_scores']
        valid_counts = (ori_scores != -1).sum(axis=1)
        

        all_hps_scores = ori_scores[np.arange(len(ori_scores)), valid_counts - 1]
        hps_mean = all_hps_scores.mean()
        ...
        if config.per_prompt_stat_tracking:
            ...
            advantages = stat_tracker.update(prompts, dummy_rewards, reward_tensors=gathered_rewards['scores'],attr_binding=config.train.attr_binding)
