...
class DistributedKRepeatSampler(Sampler):
    def __init__(self, dataset, batch_size, k, num_replicas, rank, seed=0):
        ...
        self.epoch = 0
        print(f"k : {k}, num_replicas : {num_replicas}, batch_size : {batch_size}, total_samples : {self.total_samples}, m : {self.m}")
...
def main(_):
    ...
    config.prompt_fn = "geneval"  # FORCE GENEVAL PROMPT FUNCTION
    ...
    while True:
        ...
        if config.per_prompt_stat_tracking:
            ...
            import copy
            prompts_ref = copy.deepcopy(prompts)
            ...
            if accelerator.is_local_main_process:
                dic = {}
                for i, e in enumerate(prompts_ref):

                    if e not in dic:
                        dic[e] = 1
                    else:
                        dic[e] += 1
                for idx , (i, e)  in enumerate(dic.items()):
                    print(f"{idx}. {i} : {e}")
